...
const ManageCamps = () => {
  ...
  const { data: camps = [] ,isLoading} = useQuery({
    ...
    queryFn: async () => {
      const res = await axiosSecure(`/mycamps?email=${user?.email}`);
      return res.data;
    }
  });
  ...
  return (
    <section className="p-6 max-w-7xl mx-auto">
      <h2 className="text-4xl font-extrabold mb-8 text-center text-green-700 tracking-tight">
        🌿 Manage Your Camps
      </h2>

      <div className="overflow-x-auto rounded-2xl backdrop-blur-md bg-white/70 shadow-2xl border border-green-100">
        <table className="min-w-full text-sm text-left">
          <thead className="bg-gradient-to-r from-green-100 to-green-200 text-green-900 uppercase tracking-wide">
            <tr>
            ...
            {camps.map(camp => (
              <tr key={camp._id} className="border-t hover:bg-green-50 transition-all duration-300">
                ...
                <td className="px-6 py-4 space-x-2 ">
                  <button onClick={() => openUpdateModal(camp)} className="mt-1 inline-flex items-center gap-1 px-3 py-1.5 rounded-full bg-blue-100 hover:bg-blue-200 text-blue-700 text-xs font-semibold transition">
                    <FaEdit className="text-sm" /> Edit
                  </button>
                  <button onClick={() => openDeleteModal(camp)} className="mt-1 inline-flex items-center gap-1 px-3 py-1.5 rounded-full bg-red-100 hover:bg-red-200 text-red-700 text-xs font-semibold transition">
            ...
            <div className="flex items-center justify-center min-h-full p-4 text-center">
              <TransitionChild as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
                <DialogPanel className="w-full max-w-2xl transform overflow-hidden rounded-3xl bg-white/80 backdrop-blur-xl shadow-2xl p-8 text-left align-middle transition-all border border-green-200">
                  <DialogTitle className="text-2xl font-extrabold mb-6 text-green-700">
                    ✏️ Update Camp
                  </DialogTitle>
                  <form onSubmit={handleSubmit(onUpdateSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <input type="file" disabled className="fancy-input bg-gray-100 cursor-not-allowed" placeholder="Image (unchanged)" />
                    ...
                    <button type="submit" className="md:col-span-2 bg-gradient-to-r from-green-500 to-green-700 text-white py-2 px-6 rounded-full hover:shadow-lg transition-all">
                      ✅ Save Changes
                    </button>
                  </form>
                </DialogPanel>
              ...
              <TransitionChild as={Fragment} enter="ease-out duration-300" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-200" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
                <DialogPanel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white/90 backdrop-blur-md border border-red-200 p-6 text-left align-middle shadow-xl transition-all">
                  ...
                  <div className="flex justify-end gap-3">
                    <button onClick={() => setIsDeleteOpen(false)} className="px-4 py-2 rounded border border-gray-300 hover:bg-gray-100 transition">
                      Cancel
                    </button>
                    <button onClick={() => deleteMutation.mutate(selectedCamp._id)} className="px-4 py-2 bg-gradient-to-r from-red-500 to-red-700 text-white rounded hover:shadow-md transition">
                      Delete
                    </button>
                  </div>
                </DialogPanel>
